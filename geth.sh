...
read -erp "请输入您想使用的网络模式("fast", "full", "snap" 或 "light") (默认: fast)" syncmode
[[ -z $syncmode ]] && readonly syncmode=fast
...
docker run -d --name geth-node --restart unless-stopped -v $geth_date:/root/.ethereum -p $geth_http_port:8545 -p $geth_ws_port:8546 -p 30303:30303 -p 30303:30303/udp ethereum/client-go --http --http.addr=0.0.0.0 --http.port=$geth_http_port --ws --ws.addr=0.0.0.0 --ws.port=$geth_ws_port --maxpeers=1024 --goerli --syncmode "$syncmode"
